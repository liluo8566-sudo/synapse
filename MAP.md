...
- **Duty hold union**: `breaker.covers(config_dir, shell)` (called by `_breaker_holds()`) ORs the breaker scope above with the sibling `<marrow config dir>/duty.json`'s `hold` (`duty_hold()`, read-only — this repo never writes duty.json, owned by cortex's `duty.py`, cortex/MAP.md §3.2). `hold` absent/corrupt → no duty hold (fail-open, same convention as the breaker itself).
- **Config: marrow only.** `[cortex.breaker]` in `~/.config/marrow/config.toml` (`enabled` / `fuse_threshold` / `window_hours` / `trip_message`), read directly by `breaker.settings()`. Deliberately NOT duplicated into the tg bridge config.
...
- Clearing is cortex-side only (`/ct-duty` / `cortex.ctl duty|resume`) — the bridge never clears the breaker or duty.json. `ctl duty tg`/`ctl wake --shell tg` also books a due-now `next_wake_at` in `<state_dir>/tg.json` and kicks this host's scheduler socket, so the released shell fires at once instead of waiting out its idle window. No clear/resume message is sent: only the auto trip announces itself.
...
### 9.2 tg + wx singleton boot
- Both `__main__.py` entries take a held, non-blocking `flock` on a boot lock (`_acquire_singleton_lock`; tg = `<data_dir>/synapse-tg.lock`, wx = `~/.config/synapse-wx/synapse-wx.lock`) instead of the old pidfile-kill dance. Second acquire on a live lock exits 1 immediately without touching the running instance. A stale lock from a dead pid is reclaimed for free — the kernel drops the flock the instant its holder's fd closes, so there is no pid probing.
