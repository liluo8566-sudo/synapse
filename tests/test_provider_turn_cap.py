...
def _assistant(output_tokens: int, request_id: str | None = None, **usage) -> str:
    ev: dict = {
        "type": "assistant",
...
def test_repeated_usage_same_request_not_double_counted():
    ...
    # 5 identical lines of 10000 each under one requestId. Naive summing would
...
def test_multi_request_turn_sums_and_triggers_cancel():
    """A turn spanning several requestIds sums the per-request max; exceeding
