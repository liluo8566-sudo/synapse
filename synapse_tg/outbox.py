"""Direct sqlite access to marrow ``outbox`` for tg outbound delivery.

Touches outbox rows/status columns only (precedent: commands/marrow_audit.py).
Delivery is at-most-once per row: a single UPDATE...WHERE status='pending' claims
a row (rowcount decides the winner across processes). No whole-row redelivery
after a crash — a stale 'claimed' row from a previous run is failed, never resent
(duplicate delivery to her phone is worse than a lost note).
"""
...
def claim_pending(db_path: str | Path | None, target: str = "tg") -> list[sqlite3.Row]:
...
def sweep_orphan_claimed(db_path: str | Path | None, target: str = "tg") -> list[int]:
